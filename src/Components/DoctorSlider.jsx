...
<Link to={item.link} >
  <div
    key={index}
    className="group card flex items-start p-6 bg-white hover:bg-[#8f1b1b] rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
  >
    <div className="icon text-[#8f1b1b] group-hover:text-white mr-4">
      <FontAwesomeIcon icon={item.icon} size="2x" />
    </div>
    <div className="content space-y-2">
      <h1 className="text-xl font-semibold text-gray-800 group-hover:text-white">{item.heading}</h1>
      <h2 className="text-sm text-gray-600 group-hover:text-white">{item.descriptions}</h2>
    </div>
  </div>
  </Link>
))}
